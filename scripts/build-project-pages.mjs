...
const UI_ICON_SVGS = {
  ...
  theme: '<svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="16" rx="2"/><path d="M3 9h18"/><path d="M9 9v11"/></svg>'
};
...
function repoHomepage(repo) {
  ...
  const homepage = meta.playground || repo.homepage;
...
function homeSpotlightCard(repo, options = {}) {
  const { badgeLabel = 'Experiment' } = options;
  ...
  return `
      <article class="spotlight-card">
        ...
        <div class="repo-top">
          ...
          <span class="feature-label">${escapeHtml(badgeLabel)}</span>
        </div>
        <p class="repo-desc">${homeDescriptionHtml(repo)}</p>
        ${repoMetaMarkup(repo)}
...
function homeLeadMarkup(repo) {
  ...
  return `
      <div class="feature-main">
        ${primaryImage ? `<figure class="feature-media"><a href="${projectPath(repo.full_name)}" aria-label="Open project page for ${escapeHtml(displayTitle(repo))}">${pictureMarkup(primaryImage.url, imageAlt, { loading: 'eager' })}</a></figure>` : ''}
...
function homeSectionHeadMarkup(sectionKey) {
  ...
  if (sectionKey === 'showcase') {
    return `<h2 id="showcase-title"><span class="title-with-icon"><span class="title-icon" aria-hidden="true">${uiIconSvg('theme')}</span><span>${escapeHtml(section.title)}</span></span></h2>
            <p>${escapeHtml(section.description)}</p>`;
  }
...
function renderHomePage(snapshot, lookup) {
  ...
  html = replaceGeneratedRegion(html, 'SHOWCASE_HEAD', homeSectionHeadMarkup('showcase'));
  html = replaceGeneratedRegion(html, 'SHOWCASE', `\n${SHOWCASE.map((fullName) => renderShowcaseCard(lookup.get(fullName))).join('\n')}\n        `);
        ...
        <div class="hero-copy">
          ...
          <div class="meta">
            ...
            ${meta.extraLinks?.some((link) => link.heroAfterRelease)
              ? meta.extraLinks.filter((link) => link.heroAfterRelease).map((link) => `<a class="button" href="${link.url}">${escapeHtml(heroLinkLabel(link))}</a>`).join('\n            ')
              : ''}
            ${meta.playground ? `<a class="button demo-btn" href="${meta.playground}"><span class="button-emoji" aria-hidden="true">🛝</span>Playground Demo</a>` : ''}
